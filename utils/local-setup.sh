...
WAIT_WC_READY=false KUBECONFIG=.kcp/admin.kubeconfig ${SCRIPT_DIR}/deploy.sh -c "none" -m
...
WAIT_WC_READY=false KUBECONFIG=.kcp/admin.kubeconfig ${SCRIPT_DIR}/deploy.sh -c ${GLBC_DEPLOY_COMPONENTS} -m
...
#Delete the glbc-user workload cluster generated by the deploy script, we aren't using it here.
KUBECONFIG=.kcp/admin.kubeconfig kubectl delete workloadcluster glbc-user
